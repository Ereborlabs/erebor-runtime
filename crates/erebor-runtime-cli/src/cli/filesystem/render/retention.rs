...
use super::{optional_name, print_json, standard_table};

pub(in crate::cli::filesystem) fn print_retention_inventory(
    ...
    match format {
        ...
        OutputFormat::Text => {
            println!("{}", retention_transaction_table(inventory.transactions()));
            println!("{}", retention_ref_table(inventory));
            println!("{}", retention_local_table(inventory));
            Ok(())
        }
    }
}

pub(in crate::cli::filesystem) fn print_retention_prune(
    prune: &FilesystemRetentionPrune,
    format: OutputFormat,
) -> Result<(), CliError> {
    match format {
        OutputFormat::Json => print_json(prune),
        OutputFormat::Text => {
            println!("{}", retention_prune_table(prune));
            Ok(())
        }
    }
}

fn retention_transaction_table(transactions: &[FilesystemRetentionTransaction]) -> Table {
    ...
    for transaction in transactions {
        table.add_row([
            ...
            retention_state(transaction.state()).to_owned(),
            optional_name(transaction.name()),
            transaction.promotion_id().to_owned(),
            String::from("-"),
            transaction_ref_count(transaction).to_string(),
        ]);
        for subtransaction in transaction.subtransactions() {
            table.add_row([
                subtransaction.handle().to_owned(),
                String::from("subtransaction"),
                retention_state(subtransaction.state()).to_owned(),
...
fn retention_ref_table(inventory: &FilesystemRetentionInventory) -> Table {
    let mut table = standard_table();
    table.set_header([
        "OWNER",
        "KIND",
    ...
    for transaction in inventory.transactions() {
        add_retention_ref_rows(&mut table, transaction.handle(), transaction.refs());
        for subtransaction in transaction.subtransactions() {
            add_retention_ref_rows(&mut table, subtransaction.handle(), subtransaction.refs());
        }
    }
    add_retention_ref_rows(&mut table, "loose", inventory.loose_refs());
    table
}

fn add_retention_ref_rows(table: &mut Table, owner: &str, refs: &[FilesystemRetainedRef]) {
    for reference in refs {
        table.add_row([
            owner.to_owned(),
            retained_ref_kind(reference.kind()).to_owned(),
            artifact_status(reference.status()).to_owned(),
            bool_text(reference.protected()).to_owned(),
            bool_text(reference.required_for_rollback()).to_owned(),
            reference.promotion_id().to_owned(),
            optional_name(reference.volume_id()),
            reference.reference().to_owned(),
        ]);
    }
}

fn retention_local_table(inventory: &FilesystemRetentionInventory) -> Table {
    let mut table = standard_table();
    table.set_header(["OWNER", "KIND", "STATUS", "PROTECTED", "ROLLBACK", "PATH"]);
    add_retention_local_rows(&mut table, "session", inventory.local_artifacts());
    for transaction in inventory.transactions() {
        add_retention_local_rows(
            &mut table,
            transaction.handle(),
...
fn add_retention_local_rows(
    ...
    for artifact in artifacts {
        table.add_row([
            owner.to_owned(),
            retained_local_kind(artifact.kind()).to_owned(),
            artifact_status(artifact.status()).to_owned(),
            bool_text(artifact.protected()).to_owned(),
            bool_text(artifact.required_for_rollback()).to_owned(),
            artifact.path().display().to_string(),
        ]);
    }
}

fn retention_prune_table(prune: &FilesystemRetentionPrune) -> Table {
    let mut table = standard_table();
    table.set_header([
        "STATUS",
        "TARGET",
