...
impl DaemonControlState {
    ...
    async fn handle_hello(
        ...
        if !hello.uses_supported_control_protocol() {
            return InvalidRequestSnafu {
                reason: String::from("unsupported daemon-control client protocol version"),
            }
            .fail();
        }
        self.write_message(
    ...
    async fn dispatch(
        ...
        let result = match envelope.message_kind.as_str() {
            ...
            KIND_CODEX_APP_SERVER_ATTACH_REQUEST => {
                self.codex_app_server_attach(stream, peer, &envelope).await
            }
            KIND_CODEX_APP_SERVER_INPUT_REQUEST => {
                self.codex_app_server_input(stream, peer, &envelope).await
            }
            KIND_CODEX_APP_SERVER_INPUT_CLOSE_REQUEST => {
                self.codex_app_server_input_close(stream, peer, &envelope)
                    .await
            }
    ...
    async fn codex_app_server_attach(
        &self,
        stream: &mut UnixStream,
        peer: PeerIdentity,
        envelope: &Envelope,
    ) -> Result<()> {
        let request: CodexAppServerAttachRequest = envelope
            .decode_typed_payload(KIND_CODEX_APP_SERVER_ATTACH_REQUEST)
            .context(IpcSnafu)?;
        self.apply_session_mutation(
            stream,
            peer,
    ...
    async fn session_terminal_resize(
        &self,
        stream: &mut UnixStream,
        peer: PeerIdentity,
        envelope: &Envelope,
    ) -> Result<()> {
        let request: SessionTerminalResizeRequest = envelope
            .decode_typed_payload(KIND_SESSION_TERMINAL_RESIZE_REQUEST)
            .context(IpcSnafu)?;
        let response = self.sessions.resize_terminal(
            peer.uid,
            &request.session_id,
            &request.input_lease_id,
            &request.client_instance_id,
    ...
    async fn codex_app_server_input(
        &self,
        stream: &mut UnixStream,
        peer: PeerIdentity,
        envelope: &Envelope,
    ) -> Result<()> {
        let request: CodexAppServerInputRequest = envelope
        ...
        let response = self.sessions.codex_app_server_input(
            peer.uid,
            &request.session_id,
            &request.input_lease_id,
            &request.client_instance_id,
    ...
    async fn codex_app_server_input_close(
        &self,
        stream: &mut UnixStream,
        peer: PeerIdentity,
        envelope: &Envelope,
    ) -> Result<()> {
        let request: CodexAppServerInputCloseRequest = envelope
            .decode_typed_payload(KIND_CODEX_APP_SERVER_INPUT_CLOSE_REQUEST)
            .context(IpcSnafu)?;
        let response = self.sessions.close_codex_app_server_input(
            peer.uid,
            &request.session_id,
            &request.input_lease_id,
            &request.client_instance_id,
...
mod tests {
    ...
    use erebor_runtime_ipc::{
        v1::{
            ...
            Envelope, GuardHello, PolicyTestRequest, DAEMON_CONTROL_PROTOCOL_VERSION,
            KIND_DAEMON_ERROR, KIND_DAEMON_HELLO, KIND_DAEMON_HELLO_ACK, KIND_DAEMON_LOGS_REQUEST,
            KIND_DAEMON_STATUS_REQUEST, KIND_DAEMON_STATUS_RESPONSE, PROTOCOL_VERSION,
        },
