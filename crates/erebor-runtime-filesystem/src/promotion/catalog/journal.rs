...
use crate::{
    ...
    promotion::ids::{promotion_manifest_ref, promotion_preimage_ref},
...
pub(super) fn append_rollback_event(
    ...
    append_event(
        ...
        &CatalogEvent::Rollback {
            ...
            refs: rollback_refs(event.promotion_id, event.ref_volumes)?,
...
fn rollback_refs(promotion_id: &str, volumes: &[String]) -> Result<Vec<CatalogRollbackRef>> {
    if volumes.is_empty() {
        return Ok(Vec::new());
    }
    let mut refs = vec![CatalogRollbackRef {
        ...
        volume_id: None,
        reference: promotion_manifest_ref(promotion_id)?,
    }];
    for volume_id in volumes {
        refs.push(CatalogRollbackRef {
            kind: String::from("promotion_preimage"),
            volume_id: Some(volume_id.clone()),
            reference: promotion_preimage_ref(promotion_id, volume_id)?,
        });
    }
    Ok(refs)
}

fn append_event(storage: &FilesystemSessionStorage, event: &CatalogEvent) -> Result<()> {
    let path = catalog_journal_path(storage);
    create_parent(&path)?;
...
fn catalog_journal_path(storage: &FilesystemSessionStorage) -> PathBuf {
    storage.root().join(CATALOG_DIR).join(CATALOG_JOURNAL_FILE)
}
