...
impl CodexAppServerService {
    pub fn register(&self, registration: CodexAppServerRegistration) -> Result<(), CodexSessionError> {
        let mut registrations = self.registrations.lock().map_err(|_error| {
    ...
    fn ledger(&self, session_id: &str) -> Result<Arc<Mutex<CodexAppServerLedger>>, CodexSessionError> {
        self.registrations
            .lock()
            .map_err(|_error| CodexSessionError::InvalidHookEvent {
                reason: String::from("Codex App Server registration table is unavailable"),
...
impl CodexAppServerLedger {
    ...
    fn accept_input(&mut self, frame: &[u8]) -> Result<CodexAppServerInput, CodexSessionError> {
        ...
        let object = payload.as_object().ok_or_else(|| protocol_error("App Server JSON-RPC payload is not an object"))?;
        ...
        let method = object.get("method").and_then(Value::as_str).ok_or_else(|| {
            protocol_error("App Server client payload must be a JSON-RPC request or notification")
        })?;
        let id = object.get("id").cloned();
        if sensitive_method(method) {
            let id = id.ok_or_else(|| protocol_error("sensitive App Server methods require an id"))?;
        ...
        if let Some(id) = id {
            ...
            if self.pending.len() >= MAX_INFLIGHT_REQUESTS || self.pending.contains_key(&key) {
                return Err(protocol_error("App Server request ledger rejected the in-flight id"));
            }
            if matches!(method, "turn/start" | "turn/steer") {
                ...
                self.context_dag.append_prompt(
                    &scope_ref,
                    &path,
                    serde_json::to_vec_pretty(&record).map_err(|error| protocol_error(error.to_string()))?,
                    "Record Codex App Server prompt ingress",
                )?;
                self.pending.insert(key, PendingPrompt { scope_ref, thread_id });
            } else {
                self.pending.insert(key, PendingPrompt { scope_ref: String::new(), thread_id: None });
            }
        }
    ...
    fn observe_output(&mut self, frame: &[u8]) -> Result<(), CodexSessionError> {
        ...
        let object = payload.as_object().ok_or_else(|| protocol_error("App Server stdout is not a JSON object"))?;
        if object.get("jsonrpc").and_then(Value::as_str) != Some("2.0") {
            return Err(protocol_error("App Server stdout JSON-RPC version must be 2.0"));
        }
        let Some(id) = object.get("id") else { return Ok(()); };
        let Some(prompt) = self.pending.remove(&request_key(id)?) else { return Ok(()); };
        if prompt.scope_ref.is_empty() { return Ok(()); }
...
fn parse_frame(frame: &[u8]) -> Result<(String, Value), CodexSessionError> {
    if frame.is_empty() || frame.len() > MAX_APP_SERVER_FRAME_BYTES || !frame.ends_with(b"\n") {
        return Err(protocol_error("App Server input must be one bounded newline-delimited frame"));
    }
    let raw = std::str::from_utf8(&frame[..frame.len() - 1])
        .map_err(|_error| protocol_error("App Server JSONL is not UTF-8"))?
        .trim_end_matches('\r')
        .to_owned();
    if raw.is_empty() { return Err(protocol_error("App Server JSONL frame is empty")); }
    let payload = serde_json::from_str(&raw).map_err(|error| protocol_error(format!("App Server JSONL is invalid: {error}")))?;
    Ok((raw, payload))
}

fn request_key(id: &Value) -> Result<String, CodexSessionError> {
    if !matches!(id, Value::String(_) | Value::Number(_)) {
        return Err(protocol_error("App Server request id must be a string or number"));
    }
