...
if [[ $entry_role_runtime_only == false && $recovered_entry_only == false ]]; then
  ...
  [[ -n $host_lifecycles ]] || {
    echo "the Mithril test binary has no Host platform tests" >&2
    exit 1
  }
  while IFS= read -r lifecycle; do
    host_output=$remote_root/platform-host-$lifecycle
    partial=$output_directory/host-$lifecycle.txt.partial
    "$provider" run "$vm_name" sudo env \
      "RUST_LOG=mithril_control=debug,mithril_node=debug" \
      "MITHRIL_TEST_ROOT=$remote_source" \
      "MITHRIL_TEST_OUTPUT=$host_output" \
      "MITHRIL_TEST_PIN=/sys/fs/bpf/$vm_name-$lifecycle-host" \
      "MITHRIL_TEST_LEASE=$host_output/owner.lock" \
      "MITHRIL_TEST_CGROUP=/sys/fs/cgroup/$vm_name-$lifecycle-host" \
