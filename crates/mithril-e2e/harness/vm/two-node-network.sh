...
run_direction() {
  ...
  "$provider" run "$peer_node" "$peer_remote/bin/mithril-network-test" \
    peer-server --ready-path "$ready" --output "$peer_result" \
...
run_direction "$vm_a" "$remote_a" "$vm_b" "$remote_b" "$address_b" node-a-to-node-b
run_direction "$vm_b" "$remote_b" "$vm_a" "$remote_a" "$address_a" node-b-to-node-a
