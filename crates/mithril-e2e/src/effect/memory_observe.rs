...
fn anonymous_exec_stays_closed<P: Platform>() -> TestResult<()> {
    ...
    env.node_ready()?;
    env.running(init.id())?;
    env.recovered(init.id(), "Observe-mode memory workload")?;
    let pid = actor.id();
    let task = env.task(pid, "Observe-mode memory actor")?;
    assert_eq!(
    ...
    let protected = effects.wait_many(
        ...
        -libc::EACCES,
        2,
        "Observe-mode executable protections",
    )?;
    ...
    actor.wait_gone(pid, "Observe-mode memory actor exit")?;
    actor.stop()?;
    init.stop()?;
    env.stop()
}
