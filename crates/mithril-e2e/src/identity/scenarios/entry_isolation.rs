...
fn liveness_role_is_isolated<P: Platform>() -> TestResult<()> {
    ...
    fs::create_dir(env.work().join("secret"))?;
    fs::write(env.work().join("secret/data"), b"application secret\n")?;
    fs::write(env.work().join("liveness.denied"), b"liveness secret\n")?;
    let gate = env.work().join("entry-gate");
    mkfifoat(CWD, &gate, Mode::RUSR | Mode::WUSR)?;
    ...
    let args = [
        "/work/secret/data",
        "/work/entry-gate",
        "/work/liveness.denied",
    ];
    let mut actor = env.add_actor("wc", &args)?;
    let mut release = actor.fifo_writer(&gate, "liveness entry FIFO", Duration::from_secs(5))?;
    let task = env.task(actor.id(), "liveness entry identity")?;
    let snap = &task.snapshot;
    assert_eq!(snap.creator_task_cookie, None);
    assert_eq!(snap.root_class.as_deref(), Some("external_runtime_root"));
    assert_eq!(
    ...
    actor.close();
    assert!(!actor
        .wait_exit("liveness policy denial", Duration::from_secs(5))?
        .success());
