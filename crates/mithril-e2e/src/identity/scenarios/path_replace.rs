...
fn replaced_child_keeps_policy<P: Platform>() -> TestResult<()> {
    ...
    env.start_control()?;
    let mut actor = env.start_actor("path_wildcards.py", &["replace"])?;
    let pid = actor.id();
    env.place(pid)?;
    ...
    let task = env.recovered(pid, "replacement actor")?;
    let seen = env
        .snapshot()?
        .recent_effects
        .into_iter()
        ...
        .collect::<BTreeSet<_>>();
    actor.send(b"replace-read\n")?;
    actor.close();
    let status = actor.wait_exit("replacement reads", Duration::from_secs(5))?;
    ...
    assert_eq!(result["initial"], libc::EACCES);
    assert_eq!(result["removed"], true);
    assert_eq!(result["created"], true);
    assert_eq!(result["replacement"], libc::EACCES);
