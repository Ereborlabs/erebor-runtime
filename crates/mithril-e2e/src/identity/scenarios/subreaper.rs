...
fn subreaper_keeps_identity<P: Platform>() -> TestResult<()> {
    ...
    let mut actor = env.add_actor("subreaper.py", &[])?;
    ...
    active(&root);
    fs::write(env.work().join("subreaper-start"), b"start\n")?;
    let mid_pid = actor.wait_child(actor.id(), "subreaper middle")?;
    actor.track(mid_pid)?;
    let child_pid = actor.wait_child(mid_pid, "subreaper child")?;
    actor.track(child_pid)?;
    let middle = env.task(mid_pid, "subreaper middle identity")?;
    let before = env.task(child_pid, "subreaper child identity")?;
    child(&middle, &root);
    child(&before, &middle);
    fs::write(env.work().join("subreaper-exit"), b"exit\n")?;
    actor.wait_gone(mid_pid, "subreaper middle exit")?;
    fs::write(env.work().join("subreaper-exec"), b"exec\n")?;
    let after = env.wait_exec(
        &mut actor,
        child_pid,
        before.snapshot.task_cookie,
        &before,
        "subreaper child exec",
    )?;
    ...
    assert_eq!(got.real_parent_task_cookie, 0);
    assert_eq!(
        (got.real_parent_host_tid, got.real_parent_host_tgid),
        (root.snapshot.host_tid, root.snapshot.host_tgid)
    );
