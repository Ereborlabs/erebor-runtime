...
- deterministic Codex adapter protocol behavior; and
- state-projected real Codex behavior only when Phase 5 is under test; and
...
- a loopback OCI registry and signed OCI catalog-artifact fixture only for
  Phase 10;
- a deterministic Codex-compatible fixture for Phase 4 and a separately
  provided real Codex fixture with user state for Phase 5; and
...
## Phase 4: Codex Adapter And Final Cutover

Use a deterministic Codex-compatible fixture and record its exact
binary/version/hash and package/install provenance. It must require no user
credentials or state source. Never put fixture inputs or hook tickets in the
evidence report.

Implemented evidence lives in
`crates/erebor-runtime-e2e/src/bin/codex_v1_fixture.rs`,
`crates/erebor-runtime-e2e/tests/codex_v1_fixture.rs`, and the privileged
`.github/scripts/daemon-phase4-codex-runtime.sh` extension to the installed
systemd probe. The fixture is loaded through `erebor agent load`; it has no
real Codex binary, login, `HOME`, or `CODEX_HOME` dependency. The privileged
two-UID Linux matrix passed its TTY, typed JSONL, EOF, cancellation, malformed
output, disconnect, hook ticket, daemon-socket-absence, artifact-revalidation,
and daemon-recovery cases. The authenticated state-backed vendor probe remains
Phase 5 work.

1. Use the root-curated Codex package fixture and explicitly enroll the
   deterministic fixture binary. Do not add local package import or signature
...
5. Create the approved typed filesystem-surface state binding from a
   preauthenticated caller-owned source. Record only its identity and snapshot
   manifest; do not copy credentials into the report, daemon configuration, or
   test environment visible to the workload.
6. Enroll the pinned real vendor Codex binary and exercise its interactive and
   App Server entrypoints through the daemon. Prove it receives only the fixed
   private `CODEX_HOME` projection with a daemon-owned lower snapshot and
   per-session upper. Prove refresh/revocation, cross-UID isolation, absence of
   the source host path and daemon socket, hook/peer attribution, App Server
   EOF/disconnect, and supported daemon-loss behavior.
