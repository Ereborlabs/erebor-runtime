...
protected effect
  ...
  -> mTLS gRPC upload to mithril-control
  -> deterministic graph/finding/authorization
...
| --- | --- |
| `TrustBundleOwner` | 1 bootstrap; 7 rotation/revocation/distribution |
...
| `PolicyCompiler` | 3 compile/sign; 7 Control operation |
...
Phase order is strict through Phase 11. Phase 12 allocation decisions may begin
after Phase 0, but a physical evaluation must wait for the owning prerequisite
named in Phase 12. Phase 12 cannot satisfy a Phase 11 core gate.
...
The validated architecture leaves the unmatched-workload hard floor
unallocated while declaring it required for the complete Hugging Face claim.
This master plan resolves that allocation explicitly in Phase 8. All other
unallocated surfaces remain evaluation-only in Phase 12 and require a new
approved implementation phase before they can ship.
...
Phases 3, 7, and 11 have no new first-owned Appendix C IDs: Phase 3 must prove
the observation/simulation form of the exact Phase 4/5 IDs, Phase 7 must prove
the named detection packages using already-owned inputs, and Phase 11 must
prove registry equality and every criterion active in the signed claim. A
