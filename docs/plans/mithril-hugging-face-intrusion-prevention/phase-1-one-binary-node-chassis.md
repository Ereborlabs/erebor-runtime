...
Protocol and ownership simplification: node registration is the sole capability report, followed by trust acknowledgement and a compact readiness report. Runtime observation is one authenticated, cgroup-scoped request/response rather than a second application handshake; the mode-0600 socket is owned by the configured Runtime UID and the accepted peer is independently checked with SO_PEERCRED. NodeChassis owns TrustCache directly because the run loop is its only owner. PlatformProbe moved to erebor-interceptor as KernelPlatformProbe; BpfPrototypeCompiler remains a private Phase 0 qualification helper because build-time compilation does not belong in the node or runtime loader.
...
Commands and exact source state covered: cargo fmt --all -- --check passed; cargo check --workspace passed; cargo clippy --workspace --all-targets --all-features -- -D warnings passed; focused Interceptor, IPC, Control, node, Runtime coexistence, Phase 0/1, and packaging tests passed. The authoritative bash .github/scripts/verify-rust-ci.sh procedure passed after the final Rust edit with host socket/process access. The Phase 0 verifier also retained SHA-256 0ba30384585468759ee16d0a1d6a957ef46f88385ce3814c217f00ba888cb457.
...
Unsupported/degraded paths: all effect-specific identity, observation, and prevention remain excluded. Phase 2 owns the first real lifecycle/task/process/exec identity BPF work; Phase 3 adds observe-only effect classifiers; Phase 4 adds non-network pre-effect enforcement; Phase 5 adds network/DNS/final-flow/packet enforcement. The Phase 0 object remains a qualification object, not a production prevention claim. Superseded direct ptrace and Browser-CDP mediation assertions are explicitly ignored with TODOs to reimplement them after the shared Interceptor replacement; accepting host-visible private paths was deliberately rejected.
Production BPF build handoff: Phase 1's development image and qualification helper compile only the disposable Phase 0 object. Before Phase 2 ships its first production BPF program, Phase 2 must make libbpf-cargo the single CI/build owner for C-to-BPF artifacts, consume the checked multi-architecture vmlinux headers, publish a digest-pinned object for packaging, and remove duplicated direct clang product-build paths. mithril-node must continue to load the prebuilt object through fully vendored libbpf-rs and must never invoke a compiler at runtime.
