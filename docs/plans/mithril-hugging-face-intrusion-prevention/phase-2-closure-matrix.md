...
| `ID-MOVED-PARENT-FORK-004` | `IdentityTestRunner::physical_probe` with `CloneIntoCgroupFixture`. | None; the controlled placement move needs the runner. | Qualified moved-parent `EACCES` result. | Partial: no operator shell is valid for the controlled fixture. |
| `ID-MOVED-TASK-EXEC-005` | `IdentityTestRunner::physical_probe` with `NativeProcessFixture`. | `native-child.sh --moved-exec`. | Qualified moved-task exec-denial result. | Recorded: source, manual procedure, and physical result exist; the phase remains blocked by other rows. |
