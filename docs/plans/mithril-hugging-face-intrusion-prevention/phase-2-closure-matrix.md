...
| `ENTRY-EXEC-001` | `physical_kubernetes_probe` covers non-TTY only. | `kubernetes-exec.sh` covers non-TTY only. | Pass for non-TTY `kubectl exec`. Missing TTY, `kubectl cp`, and identical native-child control. Approved administrative exec belongs to Phase 4. | Partial |
