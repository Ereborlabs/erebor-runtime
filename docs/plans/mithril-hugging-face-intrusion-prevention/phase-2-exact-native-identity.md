...
Status: Done. All 29 required fixtures have a source-backed Rust fixture and a
physical VM result. Each operator-runnable case has a readable shell and a
recorded manual result. The closure matrix states the exact limit for every
row. Phase 3 and Phase 4 policy, IPC, race, effect, and response fixtures are
not Phase 2 gates.
...
### D2.5 — Authorization proof and execution approval slot identity

Implement signed-envelope verification, trust/time/replay foundations and the
identity side of the generic one-use execution approval slot. This phase may
...
Completed deliverable IDs: D2.1-D2.6 are implemented, code-backed, and closed by all 29 required physical fixture results.
...
Fixture cases and exact physical results: the source allocates exactly 29 Phase 2 fixture IDs. Unit tests cover authorization-envelope identity, ABI layout, binding identity, runtime reconciliation, reference parsing, object embedding, required program/map sets, packaging, and fixture allocation. The native physical probe covers initial and external roots, direct `clone3(CLONE_INTO_CGROUP)`, native inheritance, exec continuity, reparenting, movement, restart, namespace PID/TID reuse, exact cgroup-path reuse, and cleanup. The Kubernetes extension covers the pre-existing conservative root, direct CRI exec, non-TTY and TTY `kubectl exec`, `kubectl cp`, an identical native child, the native lifecycle `sleep` action, HTTP, TCP, and gRPC readiness probes, distinct regular-init, native-sidecar, and application roots, a targeted ephemeral root in a shared PID namespace, concurrent exec-probe impersonation controls, termination-time PreStop identity, live identity across Kubernetes service and node restarts, same-name Pod/container lifetime reuse, and OCI prestart timeout and invalid-identity failure. The schema-15 entry JSON SHA-256 is ef749b5a6d2521c6bd865317ce3843bf685610d009500f6d37569c9bd26a57cc. The schema-16 lifecycle-sleep JSON SHA-256 is a62e82352a3153c65895d69265e4e0265d78ec6a76679e50a7d1f0bbcc2804fb. The schema-17 network-probe JSON SHA-256 is cbc024f56ce366a84aa2b0ffdbb7efaab58599b282d1f24295f30c08702fac07. The schema-18 container-identity JSON SHA-256 is dfb7b407b8a945c474a210fb769abbc09b03599ecb271f4c27cb9d195da92ada. The schema-19 ephemeral-identity JSON SHA-256 is ee12bc57c8431ac801ae6e06e2e55dbf75ec50692b3a594785fc0d27fabf0efc. The schema-20 probe-identity JSON SHA-256 is abead9ce84882d9ecc69853a417ef39ccd629f0df7de97e4ac0e5eebfd9190a6. The schema-21 PreStop JSON SHA-256 is 4d14142beb3671342c7c6d2c8ed8e5c9d85da730f60ef556f7783f7cd231fcee. The schema-26 restart JSON SHA-256 is d9be44d4315cd6097f9cb9eddc3514f6b1ef84aa5ddd326ad1709bc10f85eb02. The native schema-27 reuse JSON SHA-256 is a456f1f1640ea4c64d9ad09d48c8fcceec539060dfd5b3d72ef2d86778377a. The Kubernetes schema-27 reuse JSON SHA-256 is ec2e87cede2e019132abe34c412015a251c6d72b85e2af51885c471936e6209c. The schema-28 OCI prestart failure JSON SHA-256 is daa232982846a3dd0981b7771d16368497a4cebdec6f4a09bd13765c89937551. The independent runtime entries are restricted external roots with no fabricated purpose. The identical children have native parent lineage and the parent's restricted role. Each lifecycle-sleep and network-probe cgroup contained only its container init PID. The regular init, native sidecar, application, and ephemeral container have separate conservative roots and execution sets as applicable. Fixture allocation and unit coverage are not physical execution of every fixture. The closure matrix states each exact limit.
...
Restart qualification update: source commit `838fc8f` and schema-26 VM JSON
close `ENTRY-RESTART-001`. The accepted JSON is
...
The pre-existing Pod root was fail-closed unknown. A stopped direct CRI task
kept task cookie `64`, process state
`0000000000000001000000000000003e`, restricted external role `11`, and its
complete snapshot across the Kubernetes service restart, the node gap, and
node recovery. The node reported the runtime gap as unhealthy and made its
observation socket unavailable while the node was down. The same VM ran
`examples/mithril-identity-manual/restart.sh` as root. Automated and manual
postflight checks found no case Namespace, fixture, pin, node process, lease,
cgroup, or work directory. K3s was active.
...
This result does not qualify CRD delivery or any Phase 4 effect decision. At
this result point, `ENTRY-REUSE-001` and `ENTRY-STOCK-HOOK-FAILURE-002`
remained open. The later reuse result closes the first row.
...
## Identity Lifetime Reuse Result — 2026-08-18

Source commit `adafcddb9e50aca6637cd92b276e7742246b1693` extends
`IdentityTestRunner`, `NativeProcessFixture`, and
`CloneIntoCgroupFixture`. It adds no runner, map, role, or durable type.

The native schema-27 JSON is
...
Namespace PID `2` and namespace TID `3` each received fresh task, process,
and execution identity after reuse. The exact cgroup path was removed and
recreated. Its kernel cgroup ID changed from `46662` to `46741`, its binding
nonce changed, and its live interval changed.
...
ID, full container ID, cgroup path, kernel cgroup ID, binding nonce, live
interval, task identity, process identity, and execution identity all
changed.

The retained VM ran `native-pid-reuse.sh`, `native-cgroup-reuse.sh`, and
`kubernetes-reuse.sh` as root. Each shell printed `PASS`. Automated and
manual postflight checks found no case Namespace, fixture, Mithril pin, node
process, lease, cgroup, or work directory. The Kubernetes service was active.
The full Rust CI script passed.

The exact cgroup limit is allocator behavior. The physical case proves reuse
of one exact cgroup path with a new kernel cgroup ID. The production recovery
oracle also rejects a repeated cgroup ID when its live interval changes, but
the fixture does not force Linux to allocate that collision. This result
proves identity separation. It does not prove CRD delivery, policy effects,
or response attachment.

This closes `ENTRY-REUSE-001`. The following result closes the final required
row.

## Stock OCI Prestart Failure Result — 2026-08-18

Source commits `9820ca7`, `5534d1b`, and `4340601` extend the existing
`IdentityTestRunner`, physical result bundle, RuntimeClass path, binding
validator, and manual runtime owner. They add no map, role, generic runner, or
durable type.

The accepted schema-28 JSON is
...
The configured runtime produced these results before any payload ran:

- Valid container
  `c69d894cf9c27f49d5882c74a1059ab44a4220fb9f0517a5d147aaff5e97833e`
  timed out after exactly 30 seconds because Mithril did not release it.
- Container
  `544d5f45afef218c851279daeb005c0e1bfa022eb052312f017df6a1ab598021`
  was rejected after the OCI state container ID changed.
- Container
  `220dfabb9d5028dce492c04652d6fe56252fbef5659271fa95254937cbfd645d`
  was rejected after the request omitted its Pod UID.

All three runtime starts failed in the configured OCI prestart hook. None of
the three payload markers existed. The retained VM ran
`examples/mithril-identity-manual/kubernetes-stock-hook-failure.sh` as root.
The shell printed `PASS`. Automated and manual cleanup removed the case
Namespace, fixture, prestart request, Mithril pin, node process, lease, cgroup,
and work directory. The Kubernetes service was active.
...
`io.containerd.runc.v2` handler. The runner obtained the complete hook error
from the local K3s journal and joined it to the exact full container ID because
Pod status and Events did not retain the complete error during the fixture
window. This is entry-failure identity evidence. It is not CRD delivery,
held-task inspection, purpose, a protected effect, or response evidence.
