...
Status: Blocked on a capable real CRI effect run and the remaining manual
matrix. The automated privileged VM probe and the real Docker observe case
pass. No prevention claim is made.
...
Performance/capacity results: the observation ring is bounded at 4 MiB, recent userspace history at 1,024 records, compiled exact cells at 65,536, and path model at 4,096 states/64 components. The VM probe passed a 50,000-open reader-paused saturation case. The average baseline open time was 5,578 ns and the average observe open time was 6,869 ns in that VM. Active-denial saturation belongs to Phase 4, durable reader/WAL/recovery saturation to Phase 6, and final map N/N+1 plus platform capacity/latency qualification to Phase 11.
...
Remaining work in this phase: run and record a real CRI effect integration on a kernel that supplies unique mount IDs, and run the remaining manual matrix. The automated probe and real Docker case pass. Cross-namespace mount propagation/fan-out retains the explicit unsupported result in Phase 3; Phase 4 owns its physical enforcement qualification. Change State to Done only if every Phase 3 oracle passes. New classified socket/channel/device/VMA models belong behind their own Phase 0 physical/type gate rather than speculative placeholder maps.
Next phase not authorized: yes.
```

## Qualification update — 2026-08-12

The disposable VM harness completed the Phase 3 production-object probe in
`OBSERVE` mode. The probe recorded these physical results:
...
- A paused reader and 50,000 opens did not change the policy result, the
  unsupported-network hard result, or the benign control.
- The average baseline open time was 5,578 ns. The average observe open time
  was 6,869 ns. These values apply only to this VM run.
- The probe removed its fixture tree, pin root, lease file, and cgroup.

The real Docker manual case also passed. The protected container read the
...
The two minikube nodes support Phase 2 identity. Their kernel does not expose
the unique mount ID needed by the exact-file model. The node therefore rejects
the Phase 3 effect capability during preflight. This result is an explicit
unsupported-platform result, not a successful CRI exact-file effect run. The
phase stays **Blocked** until a CRI runtime case runs on a qualified kernel and
the remaining required manual cases are recorded.
