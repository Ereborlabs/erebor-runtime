...
| `D4.6` | Exact signed exceptions, atomic N/N+1 consumption, expiry, receipts, and restart-safe exception WAL behavior are physical. | Administrative exec is not advertised. The approved pre-PONR reservation, two late argv checks, fail-closed response, and critical evidence are not implemented or qualified. |
...
| --- | --- | --- |
| `ADMIN-EXEC-APPROVAL-001` | `UNSUPPORTED` | Control, admission, authorization, and one-use slot owners exist, but no administrative-exec claim is advertised. The approved reservation and two-stage kernel-owned argv verification transaction is not implemented or physically qualified. Reason: `ADMIN_EXEC_CLAIM_NOT_ADVERTISED`. |
...
| Approved administrative transaction | Implement and qualify pre-PONR slot reservation, copied-argv verification at committing-creds, installed-argv verification at successful exec, fail-closed termination, and critical tamper evidence. Apply the same per-exec transaction to declared probe entries without consuming their reusable declarations. Mutable syscall-entry argv cannot grant a role. An executable-only, broad runc, pipe, or socket exception is forbidden. |
