...
| `exact_file_objects` and mount-view maps | Node generation and reconciliation owners; BPF marks mount transitions dirty | BPF file, exec, mapping, and path hooks | Exact rows bind one generation. Dirty, missing, replaced, or ambiguous mount state cannot authorize. |
