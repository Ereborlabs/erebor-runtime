...
- The approved administrative path needs implementation and qualification.
  ...
  provisional complete argv snapshot match. Node writes the immutable expected
  chunks and verifies them before it arms the slot. The two late hooks compare
  copied and installed argv with the same chunks before slot consumption and
  role activation. A mismatch must grant no role, queue `SIGKILL` before user
  mode, and emit critical evidence. Declared probe entries must use the same
