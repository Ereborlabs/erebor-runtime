...
| `HF-NET-001` | `PASS` | Signed TCP and UDP paths on IPv4 and IPv6 succeed. The shared Host, direct-runc, and Kubernetes test denies an unclassified connect through the explicit role default. The network probe denies resolver destinations. Unrepresented families and protocols fail closed. |
...
The standard platform test replaced the unmatched-connect action in the
network probe. It passed with one shared actor, policy, and assertion body on
Host, direct runc, and Kubernetes. The network probe keeps its other actions
and fixture rows.
