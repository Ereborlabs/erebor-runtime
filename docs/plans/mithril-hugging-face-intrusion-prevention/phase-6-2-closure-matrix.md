...
| `D6.2.6` | **Correction in progress.** | The shared Host, direct-runc, and Kubernetes exception scenarios passed use exhaustion, expiry, restart recovery, private cleanup ordering, and teardown. The matching legacy fixture removal and complete suite remain open. |
...
| Runtime and policy lifecycle | **Partial** | The complete current-source run proved task replacement, desired-inventory cleanup, restart, no-root inspection, and fresh-root activation. The focused Host, direct-runc, and Kubernetes exception scenarios proved private cleanup ordering and teardown. The complete suite remains open. |
