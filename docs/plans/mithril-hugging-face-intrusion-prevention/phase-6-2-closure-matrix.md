...
| `D6.2.6` | **Correction in progress.** | The earlier fixture passed target withdrawal, complete desired inventory, live-runtime retention, exception use, expiry, the former revocation path, target retirement, restart, reconnect, and physical-session settlement. The accepted no-revoke exception cleanup requires focused lightweight and Kubernetes requalification. |
...
| Runtime and policy lifecycle | **Partial** | The complete current-source run proved task replacement, desired-inventory cleanup, restart, no-root inspection, and fresh-root activation. The accepted no-revoke exception target cleanup requires focused requalification. |
