...
WorkloadProtectionException target disappears or the request deletes
  ...
  -> durable counters and receipts remain records and cannot authorize use
...
```text
State: Not done. The current policy and exception implementation, PreparedContainer boundary, independent declared entry roles, package, automated fixture, and independent manual example are present. The current direct-runc and complete automated two-node Kubernetes procedures prove guarded live-process migration to a replacement base-policy generation. The current source keeps stable entry and canonical-route policy rows across runtime mount events. It uses a separate BPF-owned runtime cache generation for confirmed topology changes and stale ready-state repair. The lightweight and physical Kubernetes cases prove the current route replacement, cache-row retirement, evidence-stream failure, watch-compaction, network-partition, and storage-outage behavior. The approved administrative-entry transaction, late kernel-owned argument verification, version-changed Kubernetes recovery, physical direct non-CRI BPF fallback, and authorized final decommission remain Not done or Not run.
Current route-replacement result: background reconciliation can measure host-prefixed process-view routes before held OCI admission supplies its authoritative view. The prior code retained that binding and discarded the OCI routes. The lightweight case now reproduces this exact order through the production background reconciliation entry point. It returned PATH_TREE_ALLOWED before the correction. The node now excludes the explicit OCI binding from the retained set and installs the OCI routes. The complete K3s-runc result is `/tmp/mithril-cache-generation-lightweight-green-3/runc-entry-role-runtime-probe.json`. The current complete Kubernetes result is `/tmp/mithril-phase62-projected-epoch-full-kubernetes-20260906-g`. Both cases passed. The two qualification VMs and K3s cluster remain healthy and retained.
Implemented deliverable scope: D6.2.1 through D6.2.4 implement and automate `applicationEntry`, `additionalEntries`, `administrativeEntry`, and `externalRole`; the recorded Kubernetes fixture exercised the declared entries physically but does not prove the approved administrative entry under the current contract. D6.2.5 has automated intake, WAL, and capacity proof. The physical failure fixture proves Control and Node restart, storage failure, WAL retention and truncation, watch compaction and relist, network partition, and API outage. D6.2.6 through D6.2.10 passed their recorded automated Kubernetes physical cases. D6.2.13 is partial because the approved administrative transaction is not done. D6.2.11 includes the passing paired lightweight and Kubernetes known-route path walk, retained containerd gate, exact incident denial, and measured current-shape recovery. It is partial until version-changed Kubernetes recovery and physical direct non-CRI fallback pass. D6.2.12 is partial until version-changed Kubernetes recovery and authorized final decommission pass.
Files and durable owners changed: the branch contains both namespaced CRDs and their Helm package; PolicyDesiredStateOwner; PolicyRolloutOwner; the exception desired-state path; TrustBundleOwner; KubernetesNodeReadinessOwner; KubernetesAdmissionOwner; KubernetesWorkloadInventoryOwner; one append-only ControlStore for policy, exception, node session, trust, rollout, acknowledgement, evidence, coverage, and cursor transactions; generated NodePolicy and ControlHealth services; NodePolicyDeliveryOwner; ExceptionAuthorityOwner; RuntimeAdmissionClient; RuntimeAdmissionServer; ScheduledRuntimeBindingV1; bounded runtime-fact staging in WorkloadBindingOwner; the node activation and cgroup-binding paths; the stateless two-stage OCI adapter; the PreparedContainer binding ABI and BPF transition owner; current hook ownership; the retained containerd integration owner; the two-node fixture; and the independent manual example. The measured current-shape recovery gate is present. Version-changed Kubernetes recovery and authorized final decommission are not complete.
...
Automated verification: PreparedContainer ABI, application-default ABI, compiled BPF object, independent entry roles, repeated entry admission, complete desired-inventory validation, live-runtime retention, terminal pending-exec retirement, crash-safe stale-profile cleanup, node observation, binary WAL migration, capacity policy, Control connection reuse, VM-harness behavior, diff checks, and the complete non-Kubernetes VM procedure passed at their recorded source checkpoints. The current complete K3s-runc lightweight case proves the production process-view-before-OCI order, authoritative route replacement, confirmed-mutation generation advance, safe detached-activity reuse or repair, deterministic stale-cache repair, later lifecycle checks, and cleanup. The complete current-source two-node Kubernetes case proves the paired protected-start, cache-row retirement, evidence-health, and Node-projection behavior with real `kubepods` cgroups. The physical outage case proves evidence retention and replay, storage failure, network partition, API outage, Kubernetes watch compaction, and Control relist. The lightweight suites execute the Rust owners and fixture command paths. They do not read source text as a capability oracle.
Platform/kernel/runtime manifests: the Helm package contains both generated closed CRDs, separate writer and Control RBAC, the exact DaemonSet reader Role, the Control Deployment and Service, fail-closed admission webhooks, the node DaemonSet, and two atomically owned `createRuntime` hook registrations. The current fixture installed and read back the retained containerd default-runtime integration. The current result records Kubernetes v1.35.5+k3s1, containerd v2.2.3-k3s1, the scheduler-selected Node, and repository-owned scenario cleanup. The complete K3s-runc lightweight result records runc 1.4.2 and fixture cleanup. The two qualification VMs remain retained.
...
Unsupported/degraded paths: approved administrative exec is Not done because the reservation and late kernel-owned argv verification transaction is not implemented or physically qualified. Declared probe entries are not qualified against the same late verification requirement. Version-changed Kubernetes recovery, physical direct non-CRI BPF fallback, and authorized final decommission are Not run on the current changed source. Phase 7 graph and finding behavior is not present.
Remaining work in this phase: complete the administrative-exec fixture path and apply the same late kernel-owned argument transaction to declared probes. Run Kubernetes recovery with version-changed Control and Node images. Prove the physical direct non-CRI BPF fallback. Run the authorized final-decommission case.
