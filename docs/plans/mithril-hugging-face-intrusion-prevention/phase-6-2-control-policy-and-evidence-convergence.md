...
```text
State: Not done. The current policy and exception implementation, PreparedContainer boundary, independent declared entry roles, package, automated fixture, and independent manual example are present. Earlier direct-runc and complete automated two-node Kubernetes procedures prove guarded live-process migration to a replacement base-policy generation. The current source keeps stable entry and canonical-route policy rows across runtime mount events. It uses a separate BPF-owned runtime cache generation for confirmed topology changes and stale ready-state repair. The complete K3s-runc lightweight case and the bounded Kubernetes protected-start case prove the current route-replacement and cache slice. The complete two-node lifecycle, explicit unreachable-cache-row retirement, physical evidence-failure variants, watch-compaction, network-partition, storage-outage, version-changed Kubernetes recovery, and authorized final decommission remain Not done or Not run.
Current route-replacement result: background reconciliation can measure host-prefixed process-view routes before held OCI admission supplies its authoritative view. The prior code retained that binding and discarded the OCI routes. The lightweight case now reproduces this exact order through the production background reconciliation entry point. It returned PATH_TREE_ALLOWED before the correction. The node now excludes the explicit OCI binding from the retained set and installs the OCI routes. The complete K3s-runc result is `/tmp/mithril-cache-generation-lightweight-green-3/runc-entry-role-runtime-probe.json`. The bounded Kubernetes result is `/tmp/mithril-cache-generation-protected-start-green-final/protected-start-result.json`. Both cases passed. No qualification VM remains.
...
Remaining work in this phase: retire unreachable mount-cache rows without making them addressable. Complete the distribution-runc lifecycle and the complete two-node Kubernetes lifecycle. Qualify the remaining cache failure races. Complete the later administrative-exec fixture path and apply the same transaction to declared probes. Run the physical evidence-failure and outage matrix. Run a Kubernetes recovery with version-changed Control and Node images. Run the authorized final-decommission case.
