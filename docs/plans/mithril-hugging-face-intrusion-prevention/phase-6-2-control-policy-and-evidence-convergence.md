...
Fixture cases and exact physical results: the direct stock-runc application-start lane passed with runc 1.3.4. It recorded PREPARED to ACTIVE, the path-approved application entry, an application-default dependency read, no exact executable object, libc and the ELF loader absent from policy, successful exit, and owned-resource cleanup. The focused protected-start lane passed with Kubernetes v1.35.5+k3s1 and containerd 2.2.3-k3s1. It replaced Mithril and the protected Pod in the retained two-VM cluster. Fresh Pod UID `491f2f7d-4ee3-41fc-ac63-d5b5d80b6cd4` ran as container `472ae1ed6443dfb36fa646bc2e41c9ab60565c75fa2571941d17a3bdd68cd8d4` on `ubuntu-b1bfec97`. Policy revision `320cbb30d5da57262e156cfbb4823009eaec5ba67b40a5ba05b659e67d40449f` activated binding `d642e99f-15be-8162-bd10-c24f338cdc5c`. The exact admitted entry reached ACTIVE, an unlisted application action passed by default, and the explicit matching file Deny blocked its target. The result is `/tmp/phase-6-2-kubernetes-default-allow-20260825-run10/protected-start-result.json`. The remaining two-node fixture and manual example cases are Not run. The prior old-API Kubernetes run remains partial historical evidence.
