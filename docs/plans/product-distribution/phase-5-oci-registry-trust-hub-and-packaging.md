# Phase 5: OCI Registry, Trust, And Hub Contract

Status: Not started.

## Purpose

Add Docker-like pull, push, tag, and discovery without trusting tags, registry
metadata, or “safe” marketing labels as enforcement evidence.
...
- Continue using the Phase 3 Notary Project signature envelope and trust-policy
  semantics through the explicitly approved verifier backend. Registry
  transport changes where signatures are discovered, not what a verified
  signature means.
...
Extend `examples/codex-app-server` with the Phase 5 package pull, verification,
and signed OCI catalog walkthrough. Use a disposable local registry fixture and
redact registry credentials and tokens.
...
Stop after Phase 5 evidence and the result update. Wait for explicit approval
before Phase 6.
