...
- For `--create-per-exec`, permit only a root-approved fresh-session template.
  ...
  privileged resource.
- For `--join-session`, capture the route owner's terminal, App Server,
  Desktop, or controller-owned-scope context only from its authenticated live
  process/PTY/descriptor/scope evidence. Store an opaque context id and the
  lifetime handles required to expire it; do not accept a caller label, CWD,
  raw PID, argv text, environment value, or timing as the context.
...
- Keep production fanotify marks, held-exec decisions, namespace entry,
  candidate-side context derivation, and any transport interposition out of
  this phase. The service and route states report `not-active` until Phase 7
  passes.
