...
- Enforce root-configured per-UID limits for concurrent sessions, retained
  ...
  and IPC uploads before exposing the public CLI. The later Release Gate
...
- Preserve all currently real non-Codex capabilities through daemon-owned
  ...
  | `cli/start.rs` | typed ambient-surface start request; deeper lifetime consolidation remains Core Phase 5 |
...
external package admission is deferred to later Phase 10. This
is not an approval for
...
verification-report format. It is later Phase 10 work and must
not be wired into Phase 3 admission; remove or move the isolated prototype before declaring
