...
Not done: package GC leases; held-effect approval creation/consumption (the
durable approval repository exists, but no Linux physical effect is held for
approval); aggregate per-UID policy/output/evidence/approval quotas; full
attached PTY input, lease renewal, detach escape, and typed SIGINT behavior;
daemon replacements for `start`, `audit evidence-trace`, and `filesystem`; and
the remaining daemon/client/two-UID/privileged Linux e2e matrix. `dev
proxy-cdp` was removed by explicit product decision, rather than migrated.
The three remaining direct commands stay present and unchanged until a
daemon-owned replacement preserves their behavior. Local/remote package import and
Notation-backed package admission remain deferred to Phase 10.
