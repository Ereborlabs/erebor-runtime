...
- Add the deterministic Codex-compatible daemon/client e2e fixture. It must
  install the curated fixture through the public CLI, exercise both the TTY
  and App Server entrypoints, prove typed JSONL input/EOF/validation and
  disconnect behavior, and prove the workload cannot reach the daemon socket.
- Extend the shared-hook daemon/client and privileged installed-product tests
  for registration cleanup, same-UID cross-session ticket rejection, daemon
  restart/recovery, two-UID isolation, Linux guard effects, and daemon loss.
  Existing crate-local protocol tests and the compatibility probe do not prove
  those daemon lifecycle boundaries.
