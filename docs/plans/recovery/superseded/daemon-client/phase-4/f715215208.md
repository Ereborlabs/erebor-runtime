...
- Rewrote the example for package loading and daemon-owned TTY/App Server
  use. It runs the complete deterministic fixture in a disposable privileged
  Docker/systemd environment instead of installing a persistent host daemon;
  real-vendor Codex remains explicitly deferred to Phase 5. The approved TTY,
