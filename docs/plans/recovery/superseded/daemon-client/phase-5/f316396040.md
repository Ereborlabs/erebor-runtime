...
- the state-backed real-vendor Codex fixture deferred from Phase 4: provision a
  preauthenticated caller-owned source only through the typed filesystem
  surface binding, enroll the pinned vendor binary, and exercise both the TTY
  and App Server paths through `erebord`. Record the binary/package/install
  identities and state snapshot manifest, but never credentials, prompts, hook
  tickets, or user data. Prove the vendor process sees only the fixed private
  target and its per-session upper, not the source path or daemon socket;
...
- A real authenticated Codex run is accepted only through that generic state
  projection. Its credentials remain unavailable to the daemon control plane,
  other UIDs, logs, inspect output, and telemetry.
