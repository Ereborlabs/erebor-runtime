...
| `ID-CREATOR-PARENT-007` | Reparent or orphan a child after native creation. Run the creator-exit branch with [`native-child.sh --orphan`](./native-child.sh). | The immutable creator edge stays exact while the real-parent interval changes. Creator exit does not cover double forks, subreapers, namespace-init reparenting, ptrace reparenting, or PID reuse. |
