...
| `ID-CREATOR-PARENT-007` | Reparent or orphan a child after native creation. Run [`native-child.sh --orphan`](./native-child.sh), [`native-child.sh --double-fork`](./native-child.sh), [`native-child.sh --subreaper`](./native-child.sh), and [`native-child.sh --namespace-init`](./native-child.sh). | The immutable creator edge stays exact while the real-parent interval changes. PID reuse remains unqualified. A permitted ptrace topology belongs to Phase 4. |
