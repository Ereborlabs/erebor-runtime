...
[[ $# -eq 4 ]] || {
  echo "usage: sudo $0 <node.json> <container-id> <absolute-secret-path> <absolute-benign-path>" >&2
  exit 2
}
...
identity_pass "PASS: the CRI exact benign file remained readable and mappable in protect mode."
