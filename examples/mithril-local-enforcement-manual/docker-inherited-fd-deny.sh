...
try:
    ...
    if error.errno in (errno.EACCES, errno.EPERM):
        raise SystemExit(0)
raise SystemExit("descriptor acquired before activation bypassed the read decision")
' "$observation_probe_ready" "$3"
observation_release_probe
enforcement_expect_exact_denial
identity_pass "PASS: a descriptor acquired before activation could not return protected bytes."
