...
mkdir -- "/proc/$identity_init_pid/root$enforcement_bind_directory"
...
' "$observation_probe_ready" "$enforcement_bind_alias"
observation_release_probe
enforcement_expect_exact_denial
identity_pass "PASS: the pre-existing bind alias canonicalized to the same exact denial."
