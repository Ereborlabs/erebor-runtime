export const consoleData = {
  ...
  workloadPolicies: [
    { id: 'datasets-observe', workload: 'datasets-server', status: 'current', title: 'Observe untrusted dataset effects', action: 'Observe', expression: 'observe file.read, process.exec, network.connect source=dataset-worker', evidence: '2.8M exact effects · 14 days', confidence: 'Active generation 11d8' },
    { id: 'datasets-proc', workload: 'datasets-server', status: 'suggested', title: 'Block worker environment reads', action: 'Deny', expression: 'deny file.read /proc/** source=dataset-worker', evidence: 'HDF5 external-storage replay · 41 workers', confidence: 'High confidence' },
    { id: 'datasets-exec', workload: 'datasets-server', status: 'suggested', title: 'Block parser-launched commands', action: 'Deny', expression: 'deny process.exec shell source=dataset-parser', evidence: 'RefJinja replay · 0 admitted workflow matches', confidence: 'High confidence' },
    { id: 'datasets-c2', workload: 'datasets-server', status: 'suggested', title: 'Block public command polling', action: 'Deny', expression: 'deny network.connect public-dataset-command-channel source=dataset-worker', evidence: '5-second polling sequence · exact socket evidence', confidence: 'High confidence' },
    { id: 'datasets-kube', workload: 'datasets-server', status: 'suggested', title: 'Block runtime credential escalation', action: 'Deny', expression: 'deny kubernetes.token.request source=dataset-worker', evidence: 'ServiceAccount and kubelet access replay', confidence: 'Review identity scope' },
    { id: 'xet-paths', workload: 'xet-storage', status: 'current', title: 'Restrict repository file paths', action: 'Allow list', expression: 'allow file.read, file.write repository/** identity=xet-storage', evidence: '4.2M exact effects · 14 days', confidence: 'Active generation 21ba' },
    { id: 'xet-egress', workload: 'xet-storage', status: 'current', title: 'Restrict storage service egress', action: 'Allow list', expression: 'allow network.connect storage.internal:443 identity=xet-storage', evidence: 'Continuous network coverage', confidence: 'Active generation 21ba' },
    { id: 'hub-repository', workload: 'hub-api', status: 'current', title: 'Restrict repository writes', action: 'Allow list', expression: 'allow provider.repository.write admitted=hub-api', evidence: '7.6M effects · provider identity retained', confidence: 'Active generation 42c1' },
    { id: 'hub-credentials', workload: 'hub-api', status: 'current', title: 'Restrict production credentials', action: 'Deny', expression: 'deny file.read credentials/** outside=hub-api', evidence: 'Complete credential coverage', confidence: 'Active generation 42c1' },
    { id: 'db-observe', workload: 'database-router', status: 'current', title: 'Observe database client identities', action: 'Observe', expression: 'observe network.connect database.internal:5432', evidence: '984K exact effects · 14 days', confidence: 'Active generation 30d2' },
    { id: 'db-workers', workload: 'database-router', status: 'suggested', title: 'Allow admitted service clients', action: 'Allow list', expression: 'allow network.connect database.internal:5432 identity=dataset-service,hub-api', evidence: '92% of observed connections · exact workload identity', confidence: 'High confidence' },
    { id: 'db-unknown', workload: 'database-router', status: 'suggested', title: 'Block unmatched database clients', action: 'Deny', expression: 'deny network.connect database.internal:5432 identity=unmatched', evidence: '28 unmatched attempts · incident replay branch', confidence: 'High confidence' },
    { id: 'db-tools', workload: 'database-router', status: 'suggested', title: 'Block interactive database tools', action: 'Deny', expression: 'deny process.exec psql source=application-workload', evidence: '3 starts · 0 admitted workflows', confidence: 'Review operator access' },
    { id: 'spaces-observe', workload: 'spaces-builder', status: 'current', title: 'Observe build-service paths', action: 'Observe', expression: 'observe network.connect external source=spaces-builder', evidence: '1.3M effects · provider gap retained', confidence: 'Active generation 118a' },
    { id: 'spaces-transitive', workload: 'spaces-builder', status: 'suggested', title: 'Restrict shared-service relay paths', action: 'Deny', expression: 'deny network.connect shared-service-relay outside=approved-build-fetch', evidence: 'Artifactory relay replay · exact service destination', confidence: 'High confidence' },
    { id: 'spaces-public-write', workload: 'spaces-builder', status: 'suggested', title: 'Restrict public repository writes', action: 'Deny', expression: 'deny provider.repository.write visibility=public source=build-job', evidence: 'Credential relay replay · provider audit gap retained', confidence: 'Review provider coverage' },
  ],
  sessions: [
